...
<body class="min-h-screen flex flex-col bg-gradient-to-b from-[#EB85FF] via-[#FED1E7] via-30% to-white to-70% font-sans">
    ...
    <div class="flex-1">
        ...
        <div class="container mx-auto">
            <div id="success-message" class="bg-green-500 text-white p-4 rounded-lg my-4">
                {{ session('success') }}
            </div>
        </div>
        <script>
        ...
        @if(session('error'))
        <div class="container mx-auto">
            <div id="error-message" class="bg-red-500 text-white p-4 rounded-lg my-4">
                {{ session('error') }}
            </div>
        </div>
        <script>
        ...
        <div class="container mx-auto mt-10">
            ...
            <div class="bg-white shadow-lg rounded-lg p-5 mb-5 flex flex-wrap gap-4 items-center justify-between border-l-4 border-pink-400">
                <div class="flex gap-4 items-center flex-wrap">
                    <div>
                        @if($transaction->service && $transaction->service->image)
                        <img src="{{ '/storage/public/' . $transaction->service->image }}" alt="booking img" class="w-32 aspect-auto">
                        @else
                        <img src="{{ asset('img/massage.png') }}" alt="booking img" class="w-32 aspect-auto">
                        @endif

                    </div>
                    <div class="flex flex-col">
                        @isset($transaction->service)
                        <p class="text-xl font-bold">{{ $transaction->service->name }}</p>
                        @else
                        <p class="text-xl font-bold text-red-500">Layanan tidak tersedia</p>
                        @endisset
                ...
                <div class="flex gap-2">
                    ...
                    <div class="flex gap-2">
                        @if ($transaction->status == 'success' && $transaction->service)
                        <a href="{{ route('web.service', ['id' => $transaction->service->id]) }}"
                    ...
                    <button
                        class="bg-[#FF48B6] text-white px-4 py-2 rounded-lg"
                        onclick="bayar('{{ $transaction->token }}', '{{ route('web.booking.success', ['id' => $transaction->id]) }}')">Bayar</button>
                    @endif
                </div>
            </div>
            @endforeach
