                    ...
                    <form action="{{ route('auth.verifyOtp') }}" method="POST">
                        ...
                        <div class="flex justify-center items-center gap-2">
                            <input type="number" maxlength="1"
                                ...
                                name="otp[]" oninput="moveToNextInput(event, 0)"
                                pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" />
                            <input type="number" maxlength="1"
                                class="otp-input w-6 h-12 text-center text-xl border border-gray-300 rounded-full"
                                name="otp[]" oninput="moveToNextInput(event, 1)"
                                pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" />
                            <input type="number" maxlength="1"
                                class="otp-input w-6 h-12 text-center text-xl border border-gray-300 rounded-full"
                                name="otp[]" oninput="moveToNextInput(event, 2)"
                                pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" />
                            <input type="number" maxlength="1"
                                class="otp-input w-6 h-12 text-center text-xl border border-gray-300 rounded-full"
                                name="otp[]" oninput="moveToNextInput(event, 3)"
                                pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" />
                            <input type="number" maxlength="1"
                                class="otp-input w-6 h-12 text-center text-xl border border-gray-300 rounded-full"
                                name="otp[]" oninput="moveToNextInput(event, 4)"
                                pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" />
                            <input type="number" maxlength="1"
                                class="otp-input w-6 h-12 text-center text-xl border border-gray-300 rounded-full"
                                name="otp[]" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" oninput="moveToNextInput(event, 5)"
                                pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==1) return false;" />
                        </div>
                        ...
                        <!-- <a href="{{ route('auth.register') }}"
                            class="text-center text-[#0E1626] bg-white rounded-full w-full py-3 font-semibold my-3 block shadow-xl">
                            Resend OTP</a> -->
                    </form>

                    <form action="{{ route('auth.resendOtp') }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="text-center text-[#0E1626] bg-white rounded-full w-full py-3 font-semibold my-3 block shadow-xl">
                            Resend OTP
